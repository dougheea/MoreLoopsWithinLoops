...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 16
    answer = largest_number(([1, 4, 7], [16, 3, 12], [1]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        sub_seq = seq_seq[k]
        if len(seq_seq) > 0 and largest is None:
            if len(sub_seq) > 0:
                largest = seq_seq[k][0]
        if largest is not None:
            for j in range(len(sub_seq)):
                if sub_seq[j] > largest:
                    largest = sub_seq[j]
    return largest
...
def run_test_largest_negative_number():
    ...
    # Test 1
    expected = -2.6
    actual = largest_negative_number([(30, -5, 8, -20), (100, -2.6, 88, -40,
                                    -5), (400, 500)])
    print('Expected and actual are:', expected, actual)

    # Test 2
    expected = -1.6
    actual = largest_negative_number([(3, -15, 9, -10), (10, -1.6, 0, -3,
                                    -5), (4, 50)])
    print('Expected and actual are:', expected, actual)

    # Test 3
    expected = -10
    actual = largest_negative_number([(3, -15, 9, -10), (10, -16, 0, -31,
                                    -52), (-44, 50)])
    print('Expected and actual are:', expected, actual)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        ...
        for j in range(len(sub_seq)):
            if sub_seq[j] < 0:
                neg_list = neg_list + [sub_seq[j]]
    largest = neg_list[0]
    for h in range(len(neg_list)):
        if neg_list[h] > largest:
            largest = neg_list[h]
    return largest
